                            ...
                            public void onCompleted(JSONObject object, GraphResponse response) {
                                ...
                                if (AccessToken.getCurrentAccessToken() != null) {
                                    try {
                                        ...
                                        String fbToken1 = AccessToken.getCurrentAccessToken().getToken();
                                        fbEmail = email;
...
//                                                    Log.d("facebooktoken",fbToken);
    ...
    public void normalLogin(String email, final String username, final String image, String token) {
        Call<NormalRetrologin> call = WebAPI.getInstance().getApi().normalLogin("normal", token, email, username, "google");
        call.enqueue(new Callback<NormalRetrologin>() {
            @Override
            public void onResponse(Call<NormalRetrologin> call, Response<NormalRetrologin> response) {
...
//                                Toast.makeText(Join_us.this, ""+status, Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                }else {
